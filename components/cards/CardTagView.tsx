...
export function CardTagView({
  ...
  if (compact) {
    return isButton ? (
      ...
      <Link
        href={buildPath(ROUTES.TAG_BY_ID, {tagId: _id})}
        className="flex justify-between gap-2"
      >
        {Content}
      </Link>
  ...
  return (
    <Link
      href={buildPath(ROUTES.TAG_BY_ID, {tagId: _id})}
      className="shadow-light100_dark300"
    >
      <article className="background-light900_dark200 light-border flex w-full flex-col rounded-2xl border px-8 py-10 sm:w-[260px]">
        <div className="flex items-center justify-between gap-3">
          <div className="background-light900_dark200 w-fit rounded-sm px-5 py-1.5">
